...
#   Creating Guides -----
    def bipedGuide(self):
        ...
        if self.side == "L":
            LftGuide = [cmds.joint(n = self.side + '_' + i[0]+self.guide , p =i[1] ) for i in list(zip(self.guidsNam,self.guidstans))]
         
        if self.side == "R":
            LftGuide = [cmds.joint(n = self.side + '_' + i[0]+self.guide , p =i[1] ) for i in list(zip(self.guidsNam,self.guidstans))]
            startjnt = cmds.mirrorJoint( LftGuide[0], myz = 1, searchReplace = ( "L_", "R_"), mb = 1 )
...
#   Creating jnts -----
    def createIkFkJnt(self):
        ...
        guidsNewTans = [cmds.xform(self.side + '_' + self.guidsNam[i]+self.guide, q=1, t=1, ws=1 ) for i in range(len(self.guidsNam))]
        
        for d in self.jntTypes:
            
            LftGuide = [cmds.joint(n = self.side + '_' + i[0]+ d , p =i[1] ) for i in list(zip(self.guidsNam,guidsNewTans))]
            cmds.select(cl =1)
...
#   Creating Ctrls -----
    def createCtrl(self):
        ...
        LftGuide = [cmds.circle(n = self.side + '_' + i[0]+ self.ctrl ) for i in list(zip(self.guidsNam,self.guidstans))]
        cmds.select(cl =1)

        guidsNewTans = [cmds.xform(self.side + '_' + self.guidsNam[i]+self.guide, q=1, t=1, ws=1 ) for i in range(len(self.guidsNam))]

        for n in range(len(self.guidsNam)):
            [cmds.xform( self.side + '_' + i[0]+ self.ctrl , t = i[1], ro = self.ctrlRot ) for i in list(zip(self.guidsNam,guidsNewTans))]
...
#   Creating IK -----
    def createIK(self):
        ...
        cmds.select(self.side + '_' + self.guidsNam[0] + self.jntTypes[1], self.side + '_' + self.guidsNam[2] + self.jntTypes[1])
        ...
        cmds.xform( self.side + '_' + self.name + self.ikCtrl , t = guidsNewTans[2], ro = self.ctrlRot )
        cmds.xform( self.side + "_" + self.name + self.polVector , t = guidsNewTans[1] )
        
        cmds.parentConstraint(self.side + '_' + self.name + self.ikCtrl, self.side + "_" + self.name + self.IkHandle, mo = 1) 
        
        cmds.poleVectorConstraint(self.side + "_" + self.name + self.polVector, self.side + "_" + self.name + self.IkHandle , w = 1)
...
#   Creating FK -----
    def createfK(self):
        ...
        for i in self.guidsNam:
            cmds.parentConstraint(self.side + '_' + i + self.ctrl, self.side + '_' + i + self.jntTypes[2], mo = 1) 
            
        for i in range(len(self.guidsNam)-1):
            cmds.parentConstraint(self.side + '_' + self.guidsNam[i] + self.ctrl, self.side + '_' + self.guidsNam[i+1] + self.ctrl, mo = 1) 
    ...
    def switch(self):
        ...
        for i in self.guidsNam:
            cmds.parentConstraint( self.side + '_' + i + self.jntTypes[2],  self.side + '_' + i + self.jntTypes[1],  self.side + '_' + i + self.jntTypes[0], mo = 1) 
            
        for i in range(len(self.guidsNam)):
            
            cmds.connectAttr( self.pref[0] +".IKFK", self.side + '_' + self.guidsNam[i] + self.jntTypes[0] + "_parentConstraint1." + self.side + '_' + self.guidsNam[i] + self.jntTypes[2] + "W0")
            
            cmds.setAttr(self.pref[0] +".IKFK", 0)
            cmds.setAttr(self.side + '_' + self.guidsNam[i] + self.jntTypes[0] + "_parentConstraint1." + self.side + '_' + self.guidsNam[i] + self.jntTypes[1] + "W1", 1)
            cmds.setDrivenKeyframe(self.side + '_' + self.guidsNam[i] + self.jntTypes[0] + "_parentConstraint1." + self.side + '_' + self.guidsNam[i] + self.jntTypes[1] + "W1", cd = "preference.IKFK")
            
            cmds.setAttr(self.pref[0] +".IKFK", 1)
            cmds.setAttr(self.side + '_' + self.guidsNam[i] + self.jntTypes[0] + "_parentConstraint1." + self.side + '_' + self.guidsNam[i] + self.jntTypes[1] + "W1", 0)
            cmds.setDrivenKeyframe(self.side + '_' + self.guidsNam[i] + self.jntTypes[0] + "_parentConstraint1." + self.side + '_' + self.guidsNam[i] + self.jntTypes[1] + "W1", cd = "preference.IKFK")
...
#   Creating Guides -------------------------
    def finelize(self):
        ...
        if cmds.ls( "preference", assemblies =1 ) !=[]:
            ...
            cmds.group((cmds.ls( "jntGrp", "guideGrp", "ctrlGrp", "IkHandleGrp", "preference" , assemblies =1 )), n = "main")
