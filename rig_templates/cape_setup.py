...
def build_rig(asset_name="cape_rig", guide_path=None):
    ...
    fk_base = fkchain.create_fk_chain(
        count=1, basename="%s_neck_base" % asset_name, shape_scale=6, curveType="Circle"
    )
    fk1_objs = fkchain.create_fk_chain(
        count=1, basename="%s_fk_01" % asset_name, shape_scale=1.5, curveType="Sphere"
    )
    fk2_objs = fkchain.create_fk_chain(
        count=1, basename="%s_fk_02" % asset_name, shape_scale=1.5, curveType="Sphere"
    )
    fk3_objs = fkchain.create_fk_chain(
        count=1, basename="%s_fk_03" % asset_name, shape_scale=1.5, curveType="Sphere"
    )
    fk4_objs = fkchain.create_fk_chain(
        count=1, basename="%s_fk_04" % asset_name, shape_scale=1.5, curveType="Sphere"
    )
    fk5_objs = fkchain.create_fk_chain(
        count=1, basename="%s_fk_05" % asset_name, shape_scale=1.5, curveType="Sphere"
    )

    fk_cape_base = fkchain.create_fk_chain(
        count=1, basename="%s_back_base" % asset_name, shape_scale=6, curveType="Square"
    )
    fk_asset_base = fkchain.create_fk_chain(
        count=1, basename="%s_main" % asset_name, shape_scale=7, curveType="Box"
    )
