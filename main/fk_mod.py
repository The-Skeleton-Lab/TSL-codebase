    ...
    def create_fk_chain(self,base_rig_group = None, count = 1,curveType = 'Square', basename = 'temp', sub_controls =1,parent_to =None,pos = None):
        ...
        if count > 0:
            ...
            for i in range(1,count+1):
                gd_grp =  tr.create_transform(Trname='%s_%d_fk_guide'%(basename,i), make_local=False)
                gd = tr.create_transform(Trname = '%s_%d_fk_gd'%(basename,i), typ = 'guide',make_local=False,parent = gd_grp)
        ...
        if count > 0:
            ...
            for i in range(1,count+1):
                ctrl_grp =  tr.create_transform(Trname='%s_%d_fk_base_ctrl'%(basename,i), make_local=False)
