...
class fk_ribbon_mod:
    ...
    def create_chain(
        ...
        base_grp = tr.create_transform(
            Trname="%s" % basename, make_local=False)

        # createNetwork_grps
        network = tr.create_transform(
            Trname="%s_network" % basename, make_local=False, parent=base_grp
        )
        input_ntw = tr.create_transform(
            Trname="%s_input" % basename, parent=network)
        output_ntw = tr.create_transform(
            Trname="%s_output" % basename, parent=network)
        utz.object_tag(input_ntw, "input_network")
        utz.object_tag(output_ntw, "output_network")

        # create guides
        guide_grp = tr.create_transform(
            Trname="%s_guide" % basename,
            make_local=False,
            parent=base_grp,
            inheritTransform=0,
        )
        ...
        # create controls
        main_ctrl_grp = tr.create_transform(
            Trname="%s_ctrl" % basename,
            make_local=False,
            parent=base_grp,
            inheritTransform=0,
        )
        ...
        misc_grp = tr.create_transform(
            Trname="%s_misc" % basename,
            make_local=False,
            parent=base_grp,
            inheritTransform=0,
        )
        ...
        # createskeleton
        skeleton_grp = tr.create_transform(
            Trname="%s_jnt" % basename,
            make_local=False,
            parent=base_grp,
            inheritTransform=0,
        )
        ...
        if count > 0:
            for i in range(1, count):
                gd_grp = tr.create_transform(
                    Trname="%s_0%d_guide" % (basename, i), make_local=False
                )
                gd = tr.create_transform(
                    Trname="%s_0%d" % (basename, i),
                    typ="guide",
                    make_local=False,
                    parent=gd_grp,
                    guide_scale=shape_scale,
                )
                ...
                if i == 1:
                    pm.connectAttr(
                        input_ntw.parent_guide_mtx, gd_grp.offsetParentMatrix
                    )
        ...
        if count > 0:
            for i in range(1, count):
                ctrl_grp = pm.PyNode(
                    tr.create_transform(
                        Trname="%s_0%d_base_ctrl" % (basename, i), make_local=False
                    )
                )
                ctl = cm.create_control(
                    basename="%s_0%d" % (basename, i),
                    curveType=curveType,
                    sub_controls=1,
                    zgrps=0,
                    parent_to=ctrl_grp,
                    control_scale=shape_scale,
                )[1]

                tweak_ctrl_grp = pm.PyNode(
                    tr.create_transform(
                        Trname="%s_0%d_base_tweak" % (basename, i), make_local=False
                    )
                )
                tweak_ctl = cm.create_control(
                    basename="%s_0%d_tweak" % (basename, i),
                    curveType="Two Arrows",
                    sub_controls=1,
                    zgrps=0,
                    parent_to=tweak_ctrl_grp,
                    control_scale=(shape_scale - 0.22),
                    color=[0.7, 1, 0.2],
                )[1]

                pm.connectAttr(
                    ctl.worldMatrix[0], tweak_ctrl_grp.offsetParentMatrix)
                ...
                if i == 1:
                    ...
                    multMtx = pm.createNode(
                        "multMatrix", n="%s_0%d_multMtx" % (basename, i)
                    )
                    invMtx = pm.createNode(
                        "inverseMatrix", n="%s_0%d_invMtx" % (basename, i)
                    )
                    pm.connectAttr(gds[0].worldMatrix[0], multMtx.matrixIn[0])
                    pm.connectAttr(input_ntw.parent_guide_mtx,
                                   invMtx.inputMatrix)
                    pm.connectAttr(invMtx.outputMatrix, multMtx.matrixIn[1])
                    pm.connectAttr(input_ntw.parent_control_mtx,
                                   multMtx.matrixIn[2])
                    pm.connectAttr(multMtx.matrixSum,
                                   ctrl_grp.offsetParentMatrix)
                    ...
                    pm.connectAttr(gds[i - 1].worldMatrix[0],
                                   multMtx.matrixIn[0])
                    pm.connectAttr(
                        gds[i - 2].worldInverseMatrix[0], multMtx.matrixIn[1]
                    )
                    pm.connectAttr(
                        ctrls[i - 2].worldMatrix[0], multMtx.matrixIn[2])
                    pm.connectAttr(multMtx.matrixSum,
                                   ctrl_grp.offsetParentMatrix)
        ...
        if count > 0:
            for idx, i in enumerate(tweak_ctrls):
                jnt = tr.create_transform(
                    Trname="%s_0%d" % (i, idx + 1),
                    typ="joint",
                    make_local=False,
                    parent=misc_grp,
                )
        ...
        dist = (
            (gds[-1].worldMatrix.get()).translate -
            (gds[0].worldMatrix.get()).translate
        ).length()

        v_patches = (((count) - 2) * 2) + 2
        nb = pm.nurbsPlane(
            lr=dist, v=v_patches, ch=0, d=3, ax=[0, 0, 1], n=basename + "_plane"
        )
        ...
        sub_ctrl_grp = tr.create_transform(
            Trname="%s_sub" % basename,
            make_local=False,
            parent=main_ctrl_grp,
            inheritTransform=0,
        )
        ...
        for i in range(0, subctrl_no - 3):
            ...
            subc = cm.create_control(
                basename="%s_0%d_sub" % (basename, i),
                curveType=curveType,
                sub_controls=1,
                zgrps=0,
                parent_to=sub_ctrl_grp,
                control_scale=sub_csize,
                color=[0.2, 0.6, 0.6],
            )
        ...
        for idx, i in enumerate(odd_index_elements):
            ...
            dcm1 = pm.createNode("decomposeMatrix", n=str(
                ctrl_pairs[idx][0]) + "_dcm")
            dcm2 = pm.createNode("decomposeMatrix", n=str(
                ctrl_pairs[idx][1]) + "_dcm")
        ...
        utz.add_world_mtxs_to_output(
            output_ntw, ctrls[-1], custom_name="end_ctrl_out")
        utz.add_world_mtxs_to_output(
            output_ntw, gds[-1], custom_name="end_guide_out")

        utz.add_world_mtxs_to_output(
            output_ntw, ctrls[0], custom_name="start_ctrl_out")
        utz.add_world_mtxs_to_output(
            output_ntw, gds[0], custom_name="start_guide_out")
        # returns
