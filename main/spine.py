...
class spine_module:
    ...
    def create_spine(
        ...
        # chest control
        chest_ctrl_grp = pm.PyNode(
            tr.create_transform(Trname="chest_base", make_local=False)
        )
        chest_ctl = cm.create_control(
            basename="chest",
            curveType="Box",
            sub_controls=1,
            zgrps=0,
            parent_to=chest_ctrl_grp,
            control_scale=shape_scale - 0.2,
            color=[0.2, 0.7, 0.5],
        )[1]
        ...
        if count > 0:
            ...
            # chest_joint
            ches_jnt = tr.create_transform(
                Trname="chest", typ="joint", make_local=False, parent=misc_grp
            )
            skl.append(ches_jnt)
        ...
        dist = (
            (gds[-2].worldMatrix.get()).translate - (gds[0].worldMatrix.get()).translate
        ).length()
